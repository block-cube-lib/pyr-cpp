...
TEST(scope_exit_test, function)
{
  ...
  {
    [[maybe_unused]]
    auto se = pyrite::make_scope_exit(&function);
  }
...
TEST(scope_exit_test, function_object)
{
  ...
  {
    [[maybe_unused]]
    auto se = pyrite::make_scope_exit(function_object());
  }
...
TEST(scope_exit_test, lambda)
{
  ...
  {
    [[maybe_unused]]
    auto se = pyrite::make_scope_exit([]() { value = lambda_result; });
  }
...
TEST(scope_exit_test, nullptr)
{
  ...
  {
    [[maybe_unused]]
    pyrite::scope_exit<decltype(&function)> se(nullptr);
  }
  EXPECT_EQ(value, default_value);
}
} // namespace
